...
def set_publication_style():
    """Apply consistent, publication-quality matplotlib defaults (high dpi,
...
def save_publication_figure(output_path, formats):
    """Save the current matplotlib figure at publication quality in each
    requested format. The extension of output_path is ignored; one file is
    written per format (e.g. ROC_epoch3.png and ROC_epoch3.pdf). PNG is a
    high-resolution raster; PDF is scalable vector (no pixelation)."""
    stem = os.path.splitext(output_path)[0]
    for fmt in formats:
        plt.savefig(f"{stem}.{fmt}", dpi=PUB_DPI, format=fmt)
...
Examples:
  python LES-wrapper.py --checkpoint_dir ROC_Checkpoints \\
    ...
    parser.add_argument('--plot_format', type=str, default='both',
                        choices=['png', 'pdf', 'both'],
                        help='Output format(s) for plots: publication-quality PNG '
                             '(raster), PDF (vector), or both (default: both)')
...
def run_roc_analysis_internal(combined_csv_path, output_plot_path,
                              color_threshold=False, plot_formats=('png',)):
    ...
    (it appears in the summary table) but is not annotated on the plot. The plot
    is saved at publication quality in each format in plot_formats.
    """
    prs_probs, rrs_probs = [], []
    ...
    # Show only AUC and Best F1 on the curve (no threshold value).
    legend_text = f'AUC = {roc_auc:.3f}, Best F1 = {best_f1:.3f}'
    ax.legend([legend_text], loc="lower right")
    ax.grid(True, linestyle='--', linewidth=0.5, alpha=0.7)

    plt.tight_layout()
    save_publication_figure(output_plot_path, plot_formats)
    plt.close(fig)
...
def plot_metric_trajectory(epochs, values, metric_name, output_path, les_value,
    ...
    plt.tight_layout()
    save_publication_figure(output_path, plot_formats)
    plt.close()


def plot_combined_trajectories(epochs, auc_vals, f1_vals, thresh_vals, output_path,
                               les_values, plot_formats=('png',)):
...
def main():
    ...
    # Resolve requested plot formats ('both' -> PNG + PDF).
    plot_formats = ('png', 'pdf') if args.plot_format == 'both' else (args.plot_format,)
